<template>
  <div>
    <v-dialog v-model="editDialog" max-width="500" persistent>
      <v-card>
        <v-card-title primary-title>Edit Supplier</v-card-title>
        <v-card-text>
          <v-col cols="12">
            <v-text-field
    ...
    <v-dialog v-model="deleteDialog" max-width="350" persistent>
      <v-card>
        <v-card-text>Do you want to delete this supplier?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
        ...
        <v-data-table :headers="headers" :items="suppliers" :search="search">
          ...
          <template #item.actions="{item}">
            <div class="d-flex flex-row">
              ...
              <router-link :to="{ name: 'supplierview', params: { id: item.id }}" style="text-decoration:none">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
